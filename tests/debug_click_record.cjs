  ...
  try {
    ...
    const beforeClick = await send('Runtime.evaluate', {
      expression: `(() => {
        ...
        if (!recBtn) return { found: false };
        recBtn.click();
        return { found: true, textBefore: recBtn.innerText };
      })()`,
      returnByValue: true,
    });
    console.log('Before click:', beforeClick.result.value);

    // Wait 1.5 seconds and inspect again
    await new Promise((r) => setTimeout(r, 1500));
    const afterClick = await send('Runtime.evaluate', {
      expression: `(() => {
        const btns = Array.from(document.querySelectorAll('button'));
        const recBtn = btns.find(b => b.title && b.title.includes('recording') || b.innerText.includes('RECORD'));
        const errDiv = document.querySelector('.bg-\\\\[var\\\\(--threat-coral\\\\)\\\\]\\\\/10');
        return {
          recBtnText: recBtn ? recBtn.innerText : 'NOT_FOUND',
          recBtnTitle: recBtn ? recBtn.title : '',
          recBtnClass: recBtn ? recBtn.className : '',
          pageErrorText: errDiv ? errDiv.innerText : null
        };
      })()`,
      returnByValue: true,
    });
    console.log('After click inspection:\n', JSON.stringify(afterClick.result.value, null, 2));
